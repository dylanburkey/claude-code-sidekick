<script>
  ...
  onMount(async () => {
    ...
    if (res.ok) {
      ...
      const hooksNode = data.tree?.find((n) => n.name === 'hooks');
      tree = hooksNode?.children ?? [];
    }
  });
...
<svelte:head>
  <title>Hooks — Claude Sidekick</title>
</svelte:head>

<div class="file-editor">
  <header class="file-editor__header">
    <h1 class="file-editor__title">Hooks</h1>
    <p class="file-editor__subtitle">Browse and edit hook definition files from .claude/hooks/.</p>
  </header>

  <div class="file-editor__body">
    <aside class="file-editor__sidebar" aria-label="Hook files">
    ...
    <div class="file-editor__content">
      ...
      {:else}
        <div class="file-editor__placeholder">
          <p>Select a hook file from the sidebar.</p>
        </div>
      {/if}
    </div>
  </div>
